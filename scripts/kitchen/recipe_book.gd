## Recetario que muestra todas las recetas del juego organizadas por categoría.
## El jugador puede navegar entre recetas y consultar ingredientes y preparación.
## Se abre desde la escena de cocina al pulsar el libro de recetas.
extends Control
...
# Las recetas de repostería se separan en tartas y galletas por el prefijo del ID.
...
# Actualiza el modulate de las pestañas para indicar cuál está activa.
...
# Cambia la categoría activa, resetea el índice y muestra la primera receta.
# [param play_sound] false al inicializar para no reproducir sonido en _ready().
...
# Navega a la receta anterior si no estamos en la primera.
...
# Navega a la receta siguiente si no estamos en la última.
...
# Cierra el recetario al hacer clic izquierdo sobre el overlay.
