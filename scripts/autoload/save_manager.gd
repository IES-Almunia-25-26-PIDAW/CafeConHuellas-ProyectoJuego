## Autoload singleton que gestiona el guardado y cargado de partidas.
## Usa archivos JSON en "user://saves/" para persistir el estado del juego.
## Soporta múltiples slots y versioning para migrar saves antiguas.
## [br]
## Nota: JSON no distingue int de float, por lo que _load_json() castea
## manualmente los campos numéricos tras el parseo.
extends Node
...
## Se emite después de que se cargue correctamente de un slot.
...
func _ready() -> void:
	...
	# make_dir_recursive_absolute crea los directorios si es necesario y no existen todavía
	DirAccess.make_dir_recursive_absolute(SAVE_DIR)


# ===== PUBLIC API =====

## Guarda el estado actual del juego en la slot indicada.
## Lee los datos de GameState y los escribe en un archivo JSON.
func save_game(slot: int) -> void:
	_save_json(slot)
	game_saved.emit(slot)

## Carga el estado del juego desde la slot indicada.
## Lee el archivo y escribe todos los datos nuevamente en el GameState autoload.
## Devuelve true si la carga fue correcta, false si falló.
...
## Devuelve true si existe una savefile en la slot indicada.
## Solo revisa la extensión usada (json).
func has_save(slot: int) -> bool:
	return FileAccess.file_exists(_get_path(slot, "json"))

## Devuelve un diccionario con "day" y "timestamp" de la slot indicada.
## Devuelve un diccionario vacío si la slot está vacía o no se puede leer.
...
## Elimina la savefile de la slot indicada.
...
func _save_json(slot: int) -> void:
	...
	# Escribe el string JSON en el archivo. El archivo se cierra automáticamente al salir del scope.
